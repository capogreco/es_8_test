...
export function channelReducer(state = defaultChannelState, action) {
  switch (action.type) {
    ...
    case CHANNEL_ACTION_TYPES.SET_PATTERN: {
      ...
      const newPattern = adjustPatternLength(
        action.payload,
    ...
    case CHANNEL_ACTION_TYPES.TOGGLE_STEP: {
      ...
      if (
        typeof index !== "number" || index < 0 || index >= effectiveSubdivision
      ) {
        return state;
      }
    ...
    case CHANNEL_ACTION_TYPES.UPDATE_MULTIPLE: {
      ...
      if ("usePolyrhythm" in action.payload) {
        newState = handlePolyrhythmTransition(
          newState,
          action.payload.usePolyrhythm,
        );
      }
      ...
      if ("customSubdivision" in action.payload && newState.usePolyrhythm) {
        const newCustomSubdivision = validateCustomSubdivision(
          action.payload.customSubdivision,
        );
        newState = {
          ...newState,
          customSubdivision: newCustomSubdivision,
      ...
      if (
        "pattern" in action.payload && Array.isArray(action.payload.pattern)
      ) {
        const effectiveSubdivision = newState.usePolyrhythm
          ? newState.customSubdivision
          : newState.subdivision;
...
export const channelSelectors = {
  ...
  getActiveStepCount: (state) => state.pattern.filter((step) => step).length,

  isValidState: (state) => {
    // Validate all state properties
    if (typeof state.active !== "boolean") return false;
    if (!["sine", "square", "triangle", "sawtooth"].includes(state.waveform)) {
      return false;
    }
    ...
    if (state.customSubdivision < 1 || state.customSubdivision > 32) {
      return false;
    }
