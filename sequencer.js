...
const sequencerState = {
  ...
  channels: Array(8).fill(null).map(() => ({
    ...
    pitches: Array(96).fill(null), // null or -36 to +36
    sh: {
      mode: "rand", // 'rand' or 'shuf'
      width: 1.0, // 0-1 (amplitude)
      values: Array(96).fill(0), // Random values for each step
    },
  })),
};
...
subdivisionsInput.addEventListener("input", (e) => {
  ...
  newGlobalSubdivisions = Math.max(2, Math.min(96, newGlobalSubdivisions));
  e.target.value = newGlobalSubdivisions;

  const oldGlobalSubdivisions = sequencerState.subdivisions;
  sequencerState.subdivisions = newGlobalSubdivisions;
  ...
  for (let channel = 0; channel < 8; channel++) {
    if (!sequencerState.channels[channel].useCustomSubdivisions) {
      ...
      sequencerState.pattern[channel] = migratePattern(
      ...
      if (
        ...
        sequencerState.channels[channel].pitches = migratePitches(
      ...
      if (
        ...
        sequencerState.channels[channel].sh.values = migrateSHValues(
          sequencerState.channels[channel].sh.values,
          oldGlobalSubdivisions,
          newGlobalSubdivisions,
        );
      }
      ...
      sequencerState.channels[channel].subdivisions = newGlobalSubdivisions;
    }
  }
...
subdivisionsInput.addEventListener("blur", (e) => {
  ...
  if (value !== sequencerState.subdivisions) {
    const oldGlobalSubdivisions = sequencerState.subdivisions;
    sequencerState.subdivisions = value;
    ...
    for (let channel = 0; channel < 8; channel++) {
      if (!sequencerState.channels[channel].useCustomSubdivisions) {
        ...
        // Update channel subdivision value
        sequencerState.channels[channel].subdivisions = value;
      }
    }
...
function createSubdivisionInfo(channel) {
  const container = document.createElement("div");
  container.className = "subdivision-info";
  ...
  container.addEventListener("click", (e) => {
    ...
    if (!isCustom) {
      ...
      sequencerState.channels[channel].useCustomSubdivisions = true;
      ...
      input.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
          ...
          sequencerState.channels[channel].useCustomSubdivisions = false;
...
function generateSHValues(channel) {
  ...
  if (es8Node) {
    es8Node.port.postMessage({
      ...
      data: {
        channel: channel,
        values: sh.values.slice(0, channelSubdivisions),
      },
    });
  }
}
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    if (lfo.waveform === "sine") {
      ...
      if (cyclePos < lfo.duty) {
        ...
        // Falling phase
        y = centerY - ((1 - cyclePos) / (1 - lfo.duty)) * amplitude * 2 +
          amplitude;
      }
    }
...
function buildGrid() {
  ...
  for (let channel = 0; channel < 8; channel++) {
    ...
    const subdivInfo = createSubdivisionInfo(channel);
    labelContainer.appendChild(subdivInfo);
...
function updateChannelSubdivisions(channel, newSubdivisions) {
  ...
  sequencerState.channels[channel].subdivisions = newSubdivisions;
...
function sendChannelPatternToWorklet(channel) {
  ...
  if (
    ...
    es8Node.port.postMessage({
      ...
      data: {
        channel: channel,
        values: sequencerState.channels[channel].sh.values.slice(
          0,
          subdivisions,
        ),
      },
...
function migratePattern(oldPattern, oldSubdivisions, newSubdivisions) {
  const newPattern = new Array(96).fill(false);
  const phaseMappings = [];

  // Calculate phase for each active cell
  for (let i = 0; i < oldSubdivisions && i < 96; i++) {
    if (oldPattern[i]) {
      const phase = i / oldSubdivisions;
      const exactNewPos = phase * newSubdivisions;
      const newIndex = Math.round(exactNewPos);
      const distance = Math.abs(exactNewPos - newIndex);

      phaseMappings.push({
        oldIndex: i,
        newIndex: Math.min(newIndex, newSubdivisions - 1),
        phase: phase,
        distance: distance,
      });
    }
  ...
  // Sort by distance (closest first), then by phase (earlier first)
  phaseMappings.sort((a, b) => {
    if (Math.abs(a.distance - b.distance) < 0.0001) {
      return a.phase - b.phase; // Earlier phase wins ties
    }
    return a.distance - b.distance; // Closest phase wins
  });

  // Place cells, skipping already occupied positions
  for (const mapping of phaseMappings) {
    if (!newPattern[mapping.newIndex]) {
      newPattern[mapping.newIndex] = true;
    }
  }
...
// Migrate pitch values using the same phase-based approach
function migratePitches(oldPitches, oldSubdivisions, newSubdivisions) {
  const newPitches = new Array(96).fill(null);
  const phaseMappings = [];

  // Calculate phase for each non-null pitch
  for (let i = 0; i < oldSubdivisions && i < 96; i++) {
    if (oldPitches[i] !== null) {
      const phase = i / oldSubdivisions;
      const exactNewPos = phase * newSubdivisions;
      const newIndex = Math.round(exactNewPos);
      const distance = Math.abs(exactNewPos - newIndex);
  ...
  // Place pitches, skipping already occupied positions
  for (const mapping of phaseMappings) {
    if (newPitches[mapping.newIndex] === null) {
      newPitches[mapping.newIndex] = mapping.value;
    }
  }
...
// Migrate S&H values using the same phase-based approach
function migrateSHValues(oldValues, oldSubdivisions, newSubdivisions) {
  const newValues = new Array(96).fill(0);

  // For S&H, we map all values (not just non-zero ones)
  for (let i = 0; i < oldSubdivisions && i < 96; i++) {
    const phase = i / oldSubdivisions;
    const exactNewPos = phase * newSubdivisions;
    const newIndex = Math.min(Math.round(exactNewPos), newSubdivisions - 1);

    // Simple mapping without collision handling (S&H values can overlap)
...
function sendPatternToWorklet() {
  ...
  for (let channel = 0; channel < 8; channel++) {
    ...
    es8Node.port.postMessage({
      ...
      data: {
        channel: channel,
        subdivisions: channelSubdivisions,
      },
    });
  ...
  try {
    ...
    es8Node.port.onmessage = (event) => {
      if (event.data.type === "stepChange") {
        updateStepIndicator(event.data.step, event.data.audioTime, event.data.channel);
    ...
    statusEl.textContent =
      `Connected • ${audioContext.sampleRate}Hz • 8 channels`;
