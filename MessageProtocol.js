...
function validateValue(value, schema, path = "") {
  ...
  if (schema.type === "number") {
    ...
    if (schema.maxItems && value.length > schema.maxItems) {
      return {
        valid: false,
        error: `${path} must have at most ${schema.maxItems} items`,
      };
    }
    ...
    if (schema.required) {
      for (const key of schema.required) {
        if (!(key in value)) {
          return {
            valid: false,
            error: `${path ? path + "." : ""}${key} is required`,
          };
        }
      }
    }
  ...
  configureChannel(channel, config) {
    ...
    if (config.pattern) {
      ...
      if (patternOps.success) {
        operations.push(
          ...patternOps.messages.map((m) => ({ type: m.type, data: m.data })),
        );
      }
    }
    ...
    if (config.pitches) {
      ...
      if (pitchOps.success) {
        operations.push(
          ...pitchOps.messages.map((m) => ({ type: m.type, data: m.data })),
        );
      }
    }
